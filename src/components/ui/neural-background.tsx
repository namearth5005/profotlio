...
const fragmentShader = `
  ...
  vec4 cppn_fn(vec2 coordinate, float in0, float in1, float in2) {
    ...
    * buf[7]
    ...
    vec4 result = vec4(buf[0].x, buf[0].y, buf[0].z, 1.0);
...
const CPPNShaderMaterial = shaderMaterial(
  { iTime: 0, iResolution: new THREE.Vector2(1, 1), uIntensity: 1.0 },
  vertexShader,
  fragmentShader
);
...
function ShaderPlane({ intensity = 1.0 }: { intensity?: number }) {
...
export default function NeuralBackground({ 
  intensity = 1.0, 
  overlay = "from-black/30 via-transparent to-black/20",
  className = "",
  animate = true
  ...
  useEffect(() => {
    ...
    // Re-measure on window resize and when content changes
    const resizeObserver = new ResizeObserver(measureDocumentHeight);
    if (document.body) {
      resizeObserver.observe(document.body);
    }
    
    window.addEventListener('resize', measureDocumentHeight);
    
    // Re-measure after a short delay to catch dynamic content
    const timeoutId = setTimeout(measureDocumentHeight, 1000);
    
    return () => {
      resizeObserver.disconnect();
      window.removeEventListener('resize', measureDocumentHeight);
  ...
  return (
    ...
    >
      ...
      <div 
        className={`pointer-events-none absolute bg-gradient-to-t ${overlay}`} 
        style={{ 
          zIndex: 2,
          top: 0,
          left: 0,
